...
for ((i=0; i<${#INSTANCE_NAMES[@]}; i++)); do
  ...
  # --maintenance-policy: 指定維護策略，這裡設定為 MIGRATE
   # --scheduling=preemptible=true,onHostMaintenance=MIGRATE: 設定實例的排程選項。這裡設定為可中斷的實例(preemptible)，並在主機維護時進行遷移(MIGRATE)。
  ...
  gcloud compute instances create $name \
    ...
    --scheduling=preemptible=true,onHostMaintenance=MIGRATE \
    --scopes=default \
